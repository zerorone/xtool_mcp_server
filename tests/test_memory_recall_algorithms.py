...
async def test_advanced_recall():
    ...
    # 测试1：语义搜索
    results1 = advanced_memory_recall(
    ...
    # 测试2：思维模式匹配
    results2 = advanced_memory_recall(
        thinking_patterns=['first_principles', 'empirical', 'pragmatic'],
        limit=5
    )
    ...
    results3 = advanced_memory_recall(
        query="system issue",
        context=context,
        context_weight=0.5,
        limit=5
    ...
    for i, result in enumerate(results4[:2], 1):
        scores = result.get('advanced_scores', {})
        print(f"{i}. 内容: {result['content'][:50]}...")
        print(f"   各项得分: 语义={scores.get('semantic', 0):.2f}, "
