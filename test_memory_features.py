...
from utils.intelligent_memory_retrieval import enhanced_save_memory, intelligent_recall_memory, get_memory_index
from utils.memory_recall_algorithms import advanced_memory_recall, get_recall_engine
...
def test_advanced_recall():
    ...
    for i, mem in enumerate(results, 1):
        ...
        print(f"   语义匹配: {scores.get('semantic', 0):.2f}, "
    ...
    print("\n思维模式匹配 (查找包含批判性分析的记忆):")
    results = advanced_memory_recall(
        thinking_patterns=["critical_analysis", "systems_thinking"],
        limit=3
    )
    for i, mem in enumerate(results, 1):
        print(f"{i}. {mem['content'][:60]}...")
