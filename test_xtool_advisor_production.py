#!/usr/bin/env python3
"""
测试 XTOOL_advisor 在生产环境中的实际使用
Test XTOOL_advisor in production environment
"""
...
def test_xtool_advisor():
    """测试 xtool_advisor 工具"""
    print("🧪 测试 Xtool Advisor 生产环境功能")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("✅ 所有测试通过！Xtool Advisor 在生产环境中工作正常")
    ...
    if hasattr(advisor, 'MANAGER_AVAILABLE') and advisor.MANAGER_AVAILABLE:
        ...
        print("  - 支持的开发阶段: 11个")
        print("  - 支持的问题类型: 10种")
        print("  - 特殊模式: AUTO, DEFAULT")
    else:
        print("\n📊 降级模式统计:")
        print("  - 管理器状态: 未加载（使用基础模式）")
