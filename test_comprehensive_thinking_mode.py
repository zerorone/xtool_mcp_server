...
class ComprehensiveThinkingModeTest:
    ...
    def test_boundary_conditions(self):
        ...
        long_query_modes = self.manager.get_modes_by_keywords(long_query)
        self.result.add_test(
            "处理超长查询字符串",
            True,  # 只要不崩溃就算通过
        )
        ...
        special_modes = self.manager.get_modes_by_keywords(special_query)
        self.result.add_test(
            "处理特殊字符查询",
            True,  # 只要不崩溃就算通过
        )
    ...
    def test_performance(self):
        ...
        for i in range(100):
            ...
            modes = self.manager.auto_select_modes(context)
